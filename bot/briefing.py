...
from bot.handlers import _md

log = logging.getLogger(__name__)


async def compose_briefing(db: DB, *, socio: str) -> str:
    """Costruisce il testo del briefing per un socio."""
    ...
    if not positions_altro:
        ...
        for p in positions_altro[:10]:
            ...
            lines.append(f"  {kind_icon} *{_md(p['topic'])}*: {_md(p['claim'])[:120]}")
    ...
    if not tasks_due:
        ...
        for t in tasks_due[:10]:
            ...
            lines.append(f"  • {_md(t['title'])} \\— {t['deadline']}{owner_str}")
    ...
    if not briefing_pending:
        ...
        for b in briefing_pending[:10]:
            lines.append(f"  • {_md(b['title'])[:100]}")
